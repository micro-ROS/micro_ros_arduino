...
cd /project
if [[ `git diff-index --name-only HEAD | grep built_packages2` ]]; then
    CHANGES=1
...
if [[ " ${PLATFORMS[@]} " =~ " opencr1 " ]]; then
    ...
    FILE=/project/src/cortex-m7/fpv5-sp-d16-softfp/libmicroros.a

    if ! [[ -f "/project/src/cortex-m7/fpv5-sp-d16-softfp/libmicroros.a" -a $CHANGES ]]; then
        echo "Building opencr1 library"
...
if [[ " ${PLATFORMS[@]} " =~ " cortex_m0 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/cortex-m0plus/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building cortex_m0 library"
...
if [[ " ${PLATFORMS[@]} " =~ " cortex_m3 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/cortex-m3/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building cortex_m3 library"
        export TOOLCHAIN_PREFIX=/uros_ws/gcc-arm-none-eabi-9-2020-q2-update
...
if [[ " ${PLATFORMS[@]} " =~ " cortex_m4 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/cortex-m4/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building cortex_m4 library"
...
if [[ " ${PLATFORMS[@]} " =~ " teensy32 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/mk20dx256/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building teensy31 library"
...
if [[ " ${PLATFORMS[@]} " =~ " teensy35 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/mk64fx512/fpv4-sp-d16-hard/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building teensy35 library"
        export TOOLCHAIN_PREFIX=/uros_ws/gcc-arm-none-eabi-5_4-2016q3/bin/arm-none-eabi-
...
if [[ " ${PLATFORMS[@]} " =~ " teensy4 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/imxrt1062/fpv5-d16-hard/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building teensy4 library"
        export TOOLCHAIN_PREFIX=/uros_ws/gcc-arm-none-eabi-5_4-2016q3/bin/arm-none-eabi-
...
if [[ " ${PLATFORMS[@]} " =~ " portenta-m7 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/cortex-m7/fpv5-d16-softfp/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building portenta-m7 library"
        export TOOLCHAIN_PREFIX=/uros_ws/gcc-arm-none-eabi-7-2017-q4-major/bin/arm-none-eabi-
...
if [[ " ${PLATFORMS[@]} " =~ " kakutef7-m7 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/cortex-m7/fpv5-sp-d16-hardfp/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building kakutef7-m7 library"
        export TOOLCHAIN_PREFIX=/uros_ws/gcc-arm-none-eabi-9-2020-q2-update/bin/arm-none-eabi-
...
if [[ " ${PLATFORMS[@]} " =~ " esp32 " ]]; then
    rm -rf firmware/build
    FILE=/project/src/esp32/libmicroros.a

    if [[ ! -f $FILE || (-f $FILE && $CHANGES -eq 1 ) ]]; then
        echo "Building esp32 library"
