// generated from rosidl_generator_c/resource/idl.h.em
// with input from geometry_msgs:msg/PolygonInstanceStamped.idl
// generated code does not contain a copyright notice

#ifndef GEOMETRY_MSGS__MSG__POLYGON_INSTANCE_STAMPED_H_
#define GEOMETRY_MSGS__MSG__POLYGON_INSTANCE_STAMPED_H_

#include "geometry_msgs/msg/detail/polygon_instance_stamped__struct.h"
#include "geometry_msgs/msg/detail/polygon_instance_stamped__functions.h"
#include "geometry_msgs/msg/detail/polygon_instance_stamped__type_support.h"

#endif  // GEOMETRY_MSGS__MSG__POLYGON_INSTANCE_STAMPED_H_
